...
def int demo()
{
	for (i = 0; i < 6; i = i + 1)
	{
		...
		if (pid == 0)
		{
			if (i == 0) {
				f1 = open("pic1.txt", "w");
				write(pic1, len(pic1), 1, f1);
				close(f1);
				return 0;
			}
			else if (i == 1) {
				f2 = open("pic2.txt", "w");
				write(pic2, len(pic2), 1, f2);
				close(f2);
				return 0;
			}
			else if (i == 2) {
				f3 = open("pic3.txt", "w");
				write(pic3, len(pic3), 1, f3);
				close(f3);
				return 0;
			}
			else if (i == 3) {
				f4 = open("pic4.txt", "w");
				write(pic4, len(pic4), 1, f4);
				close(f4);
				return 0;
			}
			else if (i == 4) {
				f5 = open("pic5.txt", "w");
				write(pic5, len(pic5), 1, f5);
				close(f5);
				return 0;
			} else {
				f6 = open("pic6.txt", "w");
				write(p.pic6, len(p.pic6), 1, f6);
				close(f6);
				return 0;
			}
		}
...
if (demo() == 1)
{
	...
	pid = fork();
	if (pid == 0) {
		execlp("cat", "cat", "pic1.txt", 0);
	}
	sleep(1);
	pid = fork();
	if (pid == 0) {
		execlp("cat", "cat", "pic2.txt", 0);
	}
	sleep(1);
	pid = fork();
	if (pid == 0) {
		execlp("cat", "cat", "pic3.txt", 0);
	}
	sleep(2);
	pid = fork();
	if (pid == 0) {
		execlp("cat", "cat", "pic4.txt", 0);
	}
	sleep(1);
	pid = fork();
	if (pid == 0) {
		execlp("cat", "cat", "pic5.txt", 0);
	}
	sleep(1);
	execlp("cat", "cat", "pic6.txt", 0);
}
