...
(*
(*Type of Array*)
let array_type arr _ =
    match arr with
     A.Array(expr) -> A.allExpr
*)


(*type  snum = IntLit of int *)
...
let rec check_expr (env : environment) = function
  ...
  (*| Ast.ArrayLit(value) ->
*)
     (*let (value, typ) = check_Expr env expr in*)
 (*   let sast_assign = *) 
 (* | Ast.ArrayLit(value) -> check_table_literal env value *)
  (*value = list of literals *)
  |Ast.ArrayLit(value) ->
        (*let keys = List.map (fun i -> (Ast.IntLit i)) (Util.range 0 (List.length value ))  in*)
        (*let values_sast = (List.map (check_expr (env : environment) ) value) in *)
        let value_first = List.hd value in (* first element *)
        let value_type = List.map ( value_first) in
        let value_size = List.length value in (* length of the array *)
        ArrayLit(value), A.Array (Int)
