...
let translate (stmts) =
  ...
  (* helper: builds function prototype *)
  ...
  (* init protoypes and globals. global maps. *)
  let prototypes = build_proto StringMap.empty main_func and
  globals = StringMap.empty in

  (* initially called with main. recursively called with other fdecls *)
  (* Fill in the body of the given function *)
  let rec build_function fdecl =
    (* build protoype. *)
    let prototypes = build_proto prototypes fdecl in
    ...
    (* build locals *)
    (* at start, locals consist only of formals. we add locals on the pass. *)
    (* if main is passed, no locals are made b/c no formals exist *)
    let locals = 
    ...
    (* helper: allocates a new global or local, depending on fdecl. *)
    ...
    let alloc_var m id t = match fdecl.A.fname with
        "main" ->
          ...
          ignore (StringMap.add id global_var globals);
      | _ -> 
          let local_var = L.build_alloca (ltype_of_typ t) id builder in
          ignore (StringMap.add id local_var locals);
    in
    ...
    let lookup_alloc id t= match fdecl.A.fname with
      "main" -> try StringMap.find id globals
          with Not_found -> alloc_var globals id t; lookup id
      | _ -> try StringMap.find id locals (* how to match ""? compiler error *)
          with Not_found -> try StringMap.find id globals
    ...
    let rec expr builder = function
      ...
      | A.Assign (id, e), t ->
        ...
        let e' = expr builder e in
          ...
          ignore (L.build_store e' (lookup_alloc id t) builder); e'
    ...
    let build _stmt = match _stmt with (* need to pass builder? error due to List.iter *)
        A.Fdecl _fdecl -> ignore(build_function _fdecl) (* recursion *) 
      | _ -> ignore(stmt builder _stmt)
    in

    (* Build the code for each stmt in the function body *)
    let builder' = List.iter build fdecl.A.body in
